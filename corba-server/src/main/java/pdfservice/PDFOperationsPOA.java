...
public abstract class PDFOperationsPOA extends org.omg.PortableServer.Servant
    implements PDFOperationsOperations, org.omg.CORBA.portable.InvokeHandler {
        ...
        switch (__method.intValue()) {
            case 0: {
                ...
                for (int i0 = 0; i0 < len0; i0++) {
                    int l = in.read_long(); pdfFiles[i0] = new byte[l]; in.read_octet_array(pdfFiles[i0], 0, l);
                }
                try {
                    byte[] $result = mergePDFs(pdfFiles);
                    out = $rh.createReply(); out.write_long($result.length); out.write_octet_array($result, 0, $result.length);
            ...
            case 1: {
                ...
                try {
                    byte[] $result = splitPDF(pdfFile, startPage, endPage);
                    out = $rh.createReply(); out.write_long($result.length); out.write_octet_array($result, 0, $result.length);
                } catch (PDFException $ex) { out = $rh.createExceptionReply(); PDFExceptionHelper.write(out, $ex); }
                break;
            }
            case 2: {
                ...
                int len2 = in.read_long(); int[] pageNumbers = new int[len2];
                for (int i = 0; i < len2; i++) pageNumbers[i] = in.read_long();
                try {
                    byte[] $result = extractPages(pdfFile, pageNumbers);
                    out = $rh.createReply(); out.write_long($result.length); out.write_octet_array($result, 0, $result.length);
                } catch (PDFException $ex) { out = $rh.createExceptionReply(); PDFExceptionHelper.write(out, $ex); }
                break;
            }
            case 3: {
                ...
                int len3 = in.read_long(); int[] pageNumbers = new int[len3];
                for (int i = 0; i < len3; i++) pageNumbers[i] = in.read_long();
                try {
                    byte[] $result = deletePages(pdfFile, pageNumbers);
                    out = $rh.createReply(); out.write_long($result.length); out.write_octet_array($result, 0, $result.length);
                } catch (PDFException $ex) { out = $rh.createExceptionReply(); PDFExceptionHelper.write(out, $ex); }
                break;
            }
            case 4: {
                ...
                try {
                    byte[] $result = addPassword(pdfFile, userPassword, ownerPassword);
                    out = $rh.createReply(); out.write_long($result.length); out.write_octet_array($result, 0, $result.length);
                } catch (PDFException $ex) { out = $rh.createExceptionReply(); PDFExceptionHelper.write(out, $ex); }
                break;
            }
            case 5: {
                ...
                try {
                    byte[] $result = convertToImages(pdfFile, dpi);
                    out = $rh.createReply(); out.write_long($result.length); out.write_octet_array($result, 0, $result.length);
                } catch (PDFException $ex) { out = $rh.createExceptionReply(); PDFExceptionHelper.write(out, $ex); }
                break;
            }
            case 6: {
                ...
                try {
                    String $result = extractText(pdfFile);
                    out = $rh.createReply(); out.write_string($result);
                } catch (PDFException $ex) { out = $rh.createExceptionReply(); PDFExceptionHelper.write(out, $ex); }
                break;
            }
            case 7: {
            ...
            case 8: {
                ...
                try {
                    int $result = getPageCount(pdfFile);
                    out = $rh.createReply(); out.write_long($result);
                } catch (PDFException $ex) { out = $rh.createExceptionReply(); PDFExceptionHelper.write(out, $ex); }
                break;
            }
            case 9: {
